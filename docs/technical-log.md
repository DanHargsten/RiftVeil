...
This file is the **detailed** project journal: class names, heuristics, file-level notes, and rationale. For a short, scan-friendly “what changed” list, see [`CHANGELOG.md`](../CHANGELOG.md) at the repo root. For import-specific pitfalls, SQL, and Cargo behaviour, see [`data-import-notes.md`](data-import-notes.md).

Entries are **newest first**, same structure as before this split.
...
- **`MatchListItemDto`** / **`MatchProjections.ToListItemDto`**: **`LeagueRegion`** from **`match.Tournament.League.Region`**. **`TournamentProjections`** list mapping includes the same field where tournament rows embed matches.
- **`frontend/src/lib/api.ts`**: **`MatchListItem.leagueRegion`**.
- **`SiteFooter`**, **`frontend/src/styles/components/site-footer.css`**, **`frontend/.env.example`**, **`frontend/src/vite-env.d.ts`**: optional **`VITE_CONTACT_EMAIL`** for a mailto link under the Riot notice.

### Changed

- **`App.tsx`**: **`AppShell`** component — **`div.app-shell`** contains **`header`**, **`main.app-main`** with **`Outlet`**, **`SiteFooter`**; all **`Route`** definitions nested under **`Route element={<AppShell />}`** so there is exactly **one** document **`main`** wrapping routed content.
- **`MatchList.tsx`** / **`match-list.css`**: date grouping (**`groupMatchesByDate`**), optional empty today row, search + tournament filter, **`useId`**-linked labels, spoiler toggle structure; **`matchSearchHaystack`** includes **`leagueRegion`**.
- **`Navbar.tsx`**, **`TournamentSidebar.tsx`**, **`MatchCard.tsx`**, **`MatchList`**, **`Game*`** components, **`routes/*`**: import order, English comments, assorted **`aria-*`** / **`type="button"`** / semantics tweaks (see file diffs).
- **`styles/index.css`**: single import tree; removed **`layout.css`** and **`typography.css`** from the bundle.
- **`reset.css`**, **`app.css`**, **`home.css`**, **`container.css`**, **`navbar.css`**, **`sidebar.css`**, **`match-detail.css`**, **`league.css`**, **`admin.css`**, **`game-details.css`**, **`badge.css`**, **`table.css`**, **`variables.css`**: layout/scroll tweaks where touched; **banner-style** block comments aligned project-wide; **`navbar.css`**: deleted all **`.import-menu*`** rules (dead after imports moved to **`/admin`**).
- **`Icons.tsx`**, **`Logos.tsx`**: formatting / removed unused **`alt`** prop on logo types.
- **`ImportController`**: **`[ProducesResponseType]`** on **`BackfillGameExternalIds`**, **`BackfillGameSides`**, **`ImportOngoingGameDetailsAsync`**.
- **`LolesportsVodEnricher`**: shortened inline comment above DB tournament overlap filter (behaviour unchanged).

### Fixed

- **`MatchReadService.GetLiveAsync`**: comment uses neutral wording for why **`Status == Live`** is mirrored with the projection heuristic.

## 2026-05-11

### Added

- **`GameObjectives.tsx`** + **`MatchDetail.tsx`**, **`match-detail.css`**, **`game-details.css`**: left objectives column (towers, dragons, baron, void grubs) from **`TeamStatsDto`**, outer grid and section **`aria-labelledby`** wiring.

### Fixed

- **`GameDetailImportService.ImportDraftEntriesAsync`**: **`PicksAndBansS7`** columns **`Team1*`** / **`Team2*`** are wiki blue/red, not local match **`Team1`** / **`Team2`**. Draft rows now map through **`ResolveTeamNumberFromPickBan`** (same **`Team1Side`** / **`Team2Side`** model as player and team stats). Rows are skipped with **`SkippedMissingSides`** when sides are missing.

### Notes (existing databases)

- Games where **`Team1Side = 'Red'`** had draft **`teamNumber`** inverted vs Leaguepedia. After deploying this fix, delete bad draft rows and re-import game details for affected tournaments, e.g.  
  `DELETE FROM "GameDraftEntries" WHERE "GameId" IN (SELECT "Id" FROM "Games" WHERE "Team1Side" = 'Red');`  
  (adjust quoting to your provider; then run **`POST /api/import/game-details`** / tournament flows as usual.)
...
- **`GameDetailImportService`**: **`FetchAllCargoPagesAsync`** over **ScoreboardPlayers**, **ScoreboardTeams**, and **PicksAndBansS7** (stable **`order by`**, configurable page size); phase delays from options. *(An earlier note on this line wrongly claimed draft **`teamNumber`** should match Cargo **`Team1`/`Team2`** column indices; **`PicksAndBansS7`** uses wiki blue/red, not local match order — see **2026-05-11**.)*
...
- Documentation: root **`README.md`** — current features (Home, leagues, match detail, admin), Leaguepedia/Lolesports data flow, tech table, repo layout, local dev steps, links to changelog and technical docs.
